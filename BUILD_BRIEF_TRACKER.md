...
Last updated: 2026-09-24 (L3 complete as step 1: L3-01 Agent Builder; L3-02 archetypes; L3-03 the cited report; L3-04 dynamic dials; L3-05 the confidence score; L3-06 refusal, the unanimity check and independent voices. Next: L6 on Drishan's call).
...
| L3-04 | Explicit occupation and character per twin: HCP prescribing fields (formulary constraints, guideline adherence, rep/channel exposure, list deprivation); patient adherence fields (regimen tolerance, health literacy, transport/cost friction, routine stability) | **Dynamic dials, built 2026-09-24** (blueprint §7.2a): the model chooses 6–12 question-specific dials per session — formulary pressure, transport friction, waiting-list fatigue, regimen tolerance … — and every twin carries a 0–10 value on each, tuned like the sentiment dials, shown as a *Dynamic* group under Sentiment in the Builder, the roster and the dial dashboard, and injected into the persona prompt. Plus the free-text `character` record on authored twins. | Built (step 1) | **Built** — Drishan's direction: *not* a typed HCP/patient schema. "We will call them dynamic dials, show below sentiment dials, and they are generic, not just patients and HCPs. LLM will choose the dials and it would be tuned just how sentiment dials are tuned." Each dial is also a **Lab split and filter** (low/mid/high, namespaced `dyn:<dial>`), so a result can be read by formulary pressure as well as by age. Still open: the Fast bank path (no LLM) carries none. |
| L3-05 | Behavioural validation harness from Minds: scored battery per archetype (role knowledge, register, refusal outside knowledge, stability across restatements) | **Built 2026-09-24** (blueprint §7.12): the battery runs as a background pass after every build — role knowledge, register, refusal on things the twin cannot know, and stability across a restatement answered in a separate context — and the blend is a 0-100 **confidence score shown beside that twin** in the roster, the debate, the Lab, one-on-one chat and the report's trace card, with the four parts, the items, its own answers and the judge's evidence behind it. | Built (step 1) | **Built** — Drishan: *"Let's all that be indetailed calculated in the background and then a confidence score shown beside their responses in LAB and debates and one on one chats."* Items are model-written per segment, not authored to a Minds standard (that is L3-01's library); two of the four parts lean on a judge model; the refusal part scores low until L3-06 lets twins say "I don't know". |
| L3-06 | Twins can say "I don't know" and hold minority positions; suppress consensus drift | **Built (step 1, blueprint §7.13, 2026-09-24):** a *what you actually know* block with a bounded licence to refuse outside their own life and job; a "not mine to answer" pair on every Lab instrument reported as its own share and kept out of every denominator; a mechanical unanimity check (lopsided headline + flat structural splits → flagged above the numbers); 30% independent voices who never read the thread plus a dissent rule for the rest. The report does not carry the verdict or refusal share yet. | Built (step 1) | Built |
...
- **2026-09-24** — Drishan: *"Behavioural labs should consider dynamic dials as well."* Done: `probe.segments_for` buckets every dynamic dial low / mid / high under a namespaced `dyn:<dial>` key, so it is both a result split and a *Who answers* filter (`_select_agents` reads the same map); `probe.split_keys(rows)` replaced the hardcoded segment tuple in all four instruments and the A/B heat-map, so a new dial needs no instrument change; the experiment sentence and the split headings name the dial. Verified against the stub: transport friction high → 0% would buy, low → 100%, on the same probe. Tests: `dynamic_dials_test.py` (16).
- **2026-09-24** — L3-05 explained (four parts, which are mechanical and which need a judge, the content cost of role knowledge, and that the refusal part is the thermometer for L3-06). Drishan's direction: run it all in the background and put a **confidence score beside the twin's responses** in the Lab, the debate and one-on-one chat. Built (blueprint §7.12): `agents/validation.py` (items per segment, two answer calls in separate contexts, one judge call, weighted 0-100 blend with unscored parts dropping out), `spawned_agents.validation` (Alembic `0013`), a background pass after every build / spawn / preset load capped at 250 twins picked round-robin across segments, `agent_validated` over the websocket, `GET /agents/{id}/validation`, `GET|POST /sessions/{id}/validation`, and a `ConfidenceBadge` with a part-by-part popover on the roster card, every debate post, Lab verbatims and live answers, chat replies and the report trace card. Found while building: `stats.agreement` cannot score a single pair (its tolerance is 10% of the observed spread, which over one pair is the gap itself) — per-twin stability is now the plain gap, and agreement is used across the population where it belongs. Verified against a stub end to end. Tests: `validation_test.py` (18). Next in L3: **L3-06** ("I don't know" and minority positions) — which the refusal score now measures.
- **2026-09-24** — L3-06 built (blueprint §7.13): the limits block, the Lab's *not mine to answer* fields reported as their own number and excluded from every denominator, the mechanical unanimity flag above the numbers, and hash-chosen independent voices in the debate with a dissent rule for the rest. Drishan then chose to skip ahead to **L6** (the TPO-native output layer) rather than L4 / L5.
- **2026-09-24** — Full end-to-end test on prod via Chrome (session "E2E test — coastal GLP-1"): research (47 pages, brief), Agent Builder (two agents, real Sonnet profile, lineup + archetype), Studio (statistics gathered from Railway, sizing funnel, 8-segment plan for 12, picker, build), ontology + scoping tagging, debate with confidence badges, purchase-intent probe with dynamic-dial splits, survey, report with citations and trace cards, Ask Report, 1:1 chat with an honest "I don't know", text ingest, Save Lineup, Admin. One defect: archetype matching cast patient segments from a GP because of place words in the role — fixed the same day.
