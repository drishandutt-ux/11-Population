...
| L1-04 | RAG retrieval scoped by project and population so a twin retrieves what its cohort plausibly knows | Every agent gets the same ranked entities and the same evidence brief. | Missing | **Answered** — why shared context is a problem (unbelievable personas, agreement as artefact, provenance breaks). No build decision. |
