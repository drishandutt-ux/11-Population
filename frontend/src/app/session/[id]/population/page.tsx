...
import { api, apiFetch, EvidenceItem, PopulationBuild, PopulationConstraints, PopulationLogEntry, PopulationSegment, QuantSource, ResearchState, Session, SimMode, WSEvent } from "@/lib/api";
...
import { ArrowLeft, Users, Sparkles, Loader2, Square, RefreshCw, Check, AlertCircle, Wand2 } from "lucide-react";
...
export default function PopulationStudioPage() {
  ...
  const approve = () => guard(async () => { if (build) { setSpawn({ current: 0, total: keptCount }); setBuild(await api.population.approve(id, build.id, { count, mode })); } });
  ...
  return (
    <div className="h-screen bg-background flex flex-col overflow-hidden">
      <header className="border-b border-border/60 px-5 py-3 flex items-center gap-3 shrink-0">
        <button onClick={() => router.push(`/session/${id}?tab=agents`)} className="text-muted-foreground hover:text-foreground transition-colors p-1 -ml-1 rounded" title="Back to Agents"><ArrowLeft className="w-4 h-4" /></button>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {agentCount > 0 && !spawn && (
            <button onClick={() => router.push(`/session/${id}?tab=agents`)} className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg border border-border/60 text-muted-foreground hover:text-foreground"><Users className="w-3.5 h-3.5" /> {agentCount} agents</button>
          )}
        </div>
      </header>
        ...
        <main className="min-h-0 xl:overflow-y-auto p-4 space-y-4 order-1 xl:order-2">
          ...
          {status === "complete" && (
            <div className="glass rounded-2xl p-4 flex items-center gap-3 border border-emerald-500/25">
              ...
              <button onClick={() => router.push(`/session/${id}?tab=agents`)} className="flex items-center gap-1.5 text-xs font-semibold px-4 py-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90"><Users className="w-3.5 h-3.5" /> View agents</button>
            </div>
          )}
        </main>
