...
2. **Gather** (optional, on by default) — Claude writes 1–3 statistics queries for the detected population; each runs against the ticked publishers with a `site:` operator through the normal engine chain (`sources.search_quant`), the best pages are read (`fetch_page`), and Claude extracts typed facts (`statistic, value, group, geography, year, quote`) plus distribution sentences. Each page becomes a `quant` evidence row (relevant pages `on_topic`, `trust_tier=high`), streams to the Ingest tab's feed as a `research_item`, and is chunked into the graph under `[SOURCE quant | publisher | url | date]`. The catalogue (`QUANT_SOURCES`): Statista, ONS, gov.uk, YouGov, US Census Bureau, Pew, Gallup, Eurostat, OECD, World Bank, Our World in Data; the default ticks follow the detected geography (`default_sources`). The Sources panel can also run an ad-hoc search at any time; facts can be ignored / re-included (the evidence exclude flag) and the plan only reads included ones. Budgets: `QUANT_MAX_PAGES` (12), `QUANT_RESULTS_PER_SOURCE` (4), `QUANT_PAGES_PER_SOURCE` (2).
