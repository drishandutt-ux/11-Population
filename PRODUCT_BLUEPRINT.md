...
| **`ask`** (`v1`) — **the generic tool** | `reasoning`, `verdict` (yes/no/mixed — "does this work for you / would you go along with it"), `strength` 0–100, `feeling` -1..1, `key_factor` (3–8 free words) — then, after the run, `theme` (the factor coded into one of ≤7 population-level themes) | Yes share with a Wilson CI; **open to it** (yes or mixed); strength and feeling with bootstrap CIs; verdict mix; **coded themes** (the driver mix, without an enum per question); segment splits; verbatims with their theme chip; one sentence |
...
│   │       │                        #   instruments/ (purchase_intent, ask, choice)
...
│   │                                #   auth_test, evidence_test, measurement_test, experiment_test, ask_test,
