...
**An instrument is a self-contained tool.** It declares its own **inputs** (`InputField`), its own answer **schema**, its own **aggregator**, its own **KPIs** (`Kpi`), its **metrics** (`Metric` — the per-answer numbers an A/B test compares, one marked `primary`), optionally a `decision_key` / `driver_key` (which answer fields count as "the decision" and "why", for flip analysis), and the **page** that renders its results. The probe runner, the statistics library and the Lab shell know nothing about any particular instrument — the same rule that keeps the debate generic (§8). Tools differ; the spine stays generic.
...
Purchase intent declares four metrics — `would_buy` (share, **primary**), `likelihood_0_100`, `max_price` (money), `sentiment` — with `decision_key="would_buy"` and `driver_key="key_driver"`, which is what makes it testable in an A/B (§11a.7).

Planned next (same primitive, per the Behaviour Lab plan): price sensitivity (Van Westendorp), budget allocation, sentiment & stance shift, Likert/NPS, conjoint, MaxDiff, Kano, message testing, brand funnel, churn, framing tests, structured interviews. Any of them becomes A/B-testable the moment it declares metrics.
...
Scenario branching (re-running the whole debate under two stimuli) is **not built**.
...
**UI — built to read as a comparison, not as two probes.** The Lab picker shows an **A/B test** card whenever any instrument declares metrics. `lab/ExperimentPanel` (the tool) — base instrument, name, design, a variant editor that renders the base tool's own `InstrumentForm` per variant (A prefilled from the session query; a new variant copies the last so the analyst edits the difference; 2–6 variants), "who answers", Fast/Pro, a live estimate (`agents × variants = calls · $`), Run. **While running** (`LiveComparison` + `PairedDots`): not one dot grid per arm but **one pill per agent split into one segment per arm**, each segment taking the colour of that agent's answer as it lands, a ring marking an agent whose decision already differs between arms; counters per arm, "answered both", and **"changed their answer" in amber**, with the latest movers named. `lab/pages/ExperimentPage` (results), in reading order: the **verdict**; the primary metric as a **before/after with one big signed delta** and its interval (`DeltaHero`); the lift on every metric (`LiftBar` drawn around zero, grey when the interval includes it); **how the same agents moved** — a flow diagram (`FlowDiagram`, from the results' `flips.matrix`: control answers on the left, variant answers on the right, ribbons per from→to group, movers solid and stayers faint) beside the coded reasons, then the movers' own words with a toggle to see what they told the control; the segment **heat-map** (`HeatCell`); and only then **the arms, one compact line each** (key, label, stimulus, headline share, n) as context. CSV: one row per agent, every arm's answer fields side by side, plus `flipped`. Past runs list experiments alongside standalone probes; arm probes are not listed on their own, and an arm completing never hijacks the shell's probe view.
...
| Method | Path | Purpose |
|---|---|---|
| GET | `/lab/instruments` | The instrument library — key, label, description, question, **`inputs`** (the tool's own declared controls), **`kpis`**, **`metrics`** + `supports_experiments` + `decision_key` (what an A/B test can compare), `page`, answer schema. The single source of truth both sides read: the UI builds its form and its headline strip from this. |
...
| POST | `/sessions/{id}/experiments/estimate` | Agents matched, number of calls (`agents × variants` within, `agents` between), model and cost — before you run. |
| POST | `/sessions/{id}/experiments` | Run an A/B/n test: `{instrument, design: within\|between, name, variants: [{key, label, spec}], spec, agent_filter, mode, seed}`. Creates the experiment and one queued probe per arm, returns both immediately; the run and the comparison happen in the background. **400** for a non-testable instrument, <2 or >6 variants, duplicate keys, a variant missing a required input, an unknown design, or no population; **404** for an unknown instrument. |
...
│   │       ├── measurement/         # Behaviour Lab: probe (runner), experiment (A/B/n),
│   │       │                        #   stats (pure Python), instruments/ (one file per instrument)
...
│   │                                #   auth_test, evidence_test, measurement_test, experiment_test,
