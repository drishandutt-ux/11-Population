...
One AI persona within a session. Columns: `id`, `session_id` (indexed), `name`, `age` (default 30), `role`, `background` (2–3 sentence bio), `stance` (enum), `correlation` (one-sentence relation to the topic), `personality` (JSON **list** of trait tags), `debate_style`, `energy` (Float, 0.3–1.0 — drives how often it posts/debates), `avatar_color` (hex), **`dials` (JSON dict — the 112-value psychological profile)**, **`humanity`** (Integer 0–100, default 0 — emotional-vs-analytical register set at spawn; see §7.7), **`verdict`** (Text, nullable — the persisted one-line Claude verdict shown in the Agent Opinions sidebar; `NULL` until generated; added via the non-destructive startup migration in `database._ensure_columns`), **`segment`** (String, nullable — the Population Studio plan segment the agent was built from, §7.8) and **`demographics`** (JSON, nullable — `{gender, region, income_band, education, occupation}` fixed by that segment; both `NULL` for Fast/Pro spawns), `created_at`.
...
**Where the demographics go afterwards.** `agent_runner._demographics_line` puts gender, place, income, education and the segment into every persona prompt (posts, chat, probes), and `probe.segments_for` adds `segment`, `gender`, `region`, `income_band` and `education` as Lab split keys, so a survey can be cut by the slice it was built from. Presets carry `segment`/`demographics`/`humanity` through save and load.
