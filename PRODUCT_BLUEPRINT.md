...
**Visibility.** The build row is re-emitted whole (`population_build`) on every state change and each log line streams as `population_log`; the page also polls every 6 s while a stage runs. A reload restores the dials the build was started with. The Studio never spawns without an explicit Approve.
...
| POST | `/sessions/{id}/population/builds/{bid}/approve` | `{count?, mode?}` — wipe the session's agents and build the roster from the non-rejected segments (background). **409** unless `awaiting_review` / `complete` / `error`; **400** with nothing to build. |
| POST | `/sessions/{id}/population/builds/{bid}/stop` | Stop the running stage. |
