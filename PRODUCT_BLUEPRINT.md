...
2. **Gather** (optional, on by default) — Claude **decomposes the question into 2–5 fact targets** (one base rate each — group sizes, distributions, adoption, attitudes), each phrased the way its publisher titles pages (dataset noun phrases for statistics offices, topic+survey for polling houses, market phrases for Statista) and routed only to the sources likely to hold it (up to 4 targets run; each is logged as "Fact target: …"). If that planning call fails, a mechanical `keyword_squeeze` strips the question to its content words (no prices, parentheticals or question scaffolding) instead of searching the raw question. The **Sources panel's ad-hoc Search** gets the same treatment: a query that reads like a question (`looks_like_question`) is decomposed before searching (a terse keyword query still runs verbatim), with the page budget shared across the sub-queries; each runs against the ticked publishers with a `site:` operator through the normal engine chain (`sources.search_quant`), the best pages are read (`fetch_page`), and Claude extracts typed facts (`statistic, value, group, geography, year, quote`) plus distribution sentences. Each page becomes a `quant` evidence row (relevant pages `on_topic`, `trust_tier=high`, `run_id` NULL — that column is a foreign key to `research_runs` on Postgres, so the gathering build is recorded as `structured.build_id`), streams to the Ingest tab's feed as a `research_item`, and is chunked into the graph under `[SOURCE quant | publisher | url | date]`. The catalogue (`QUANT_SOURCES`): Statista, ONS, gov.uk, YouGov, US Census Bureau, Pew, Gallup, Eurostat, OECD, World Bank, Our World in Data; the default ticks follow the detected geography (`default_sources`). The Sources panel can also run an ad-hoc search at any time; facts can be ignored / re-included (the evidence exclude flag) and the plan only reads included ones. Budgets: `QUANT_MAX_PAGES` (12), `QUANT_RESULTS_PER_SOURCE` (4), `QUANT_PAGES_PER_SOURCE` (2).
...
| POST | `/sessions/{id}/population/builds` | Start a build: `{mode, count, constraints, sources{quant, quant_sources[], quant_query}}`. Supersedes any active build. Runs detect → gather → clarify → plan in the background; returns the build row. |
...
*End of blueprint. For the precise behavior of any subsystem, the files above are authoritative; this document summarizes them as of 2026-09-15.*
