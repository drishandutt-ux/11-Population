...
- **Flips** (within only) — agents whose `decision_key` answer changed, with *both* reasonings, the variant answer's `driver_key`, and their segments; plus the direction mix (`no → yes`…) and the **coded reasons** (driver distribution among flippers).
...
**UI.** The Lab picker shows an **A/B test** card whenever any instrument declares metrics. `lab/ExperimentPanel` (the tool) — base instrument, name, design, a variant editor that renders the base tool's own `InstrumentForm` per variant (A prefilled from the session query; a new variant copies the last so the analyst edits the difference; 2–6 variants), "who answers", Fast/Pro, a live estimate (`agents × variants = calls · $`), Run. While running, **one dot grid per arm** fills in from the ordinary `probe_answer` events. `lab/pages/ExperimentPage` renders the results: verdict card, the arms side by side (each arm's own headline + sentence from its probe aggregates), the lift table (a `LiftBar` per metric drawn around zero, grey when the interval includes it), who flipped (direction, coded reasons, verbatims with a toggle to show what they said to the control), and the segment **heat-map** (`HeatCell`). CSV: one row per agent, every arm's answer fields side by side, plus `flipped`. Past runs list experiments alongside standalone probes; arm probes are not listed on their own, and an arm completing never hijacks the shell's probe view.
